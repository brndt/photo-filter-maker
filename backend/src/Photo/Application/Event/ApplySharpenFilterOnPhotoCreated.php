...
final class ApplySharpenFilterOnPhotoCreated
{
    ...
    {
        if (false === in_array(Filter::sharpen()->getValue(), $event->getFiltersToApply())) {
            return;
        }

        $newImageURL = $this->imageProcessing->applySharpen($event->getNameURL());
