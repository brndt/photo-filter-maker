...
namespace LaSalle\Performance\Shared\Domain\ValueObject;

use ArgumentCountError;
use BadMethodCallException;
use JsonSerializable;
use LaSalle\Performance\Shared\Domain\Exceptions\DuplicatedValueException;
use LaSalle\Performance\Shared\Domain\Exceptions\InvalidIndexException;
use LaSalle\Performance\Shared\Domain\Exceptions\InvalidNameException;
use LaSalle\Performance\Shared\Domain\Exceptions\InvalidValueException;
use ReflectionClass;
...
abstract class Enum implements Enumerable, JsonSerializable
{
        ...
        if (static::startsWith($name, 'is')) {
            if (!isset($arguments[0])) {
                throw new ArgumentCountError(
                    'Calling ' . static::class . '::' . $name . '() in static context requires one argument'
                );
            }
    ...
    public static function make($value): Enumerable
    {
        if (!is_int($value) && !is_string($value)) {
            throw new TypeError(
                static::class . '::make() expects string|int as argument but ' . gettype($value) . ' given'
            );
        }
    ...
    {
        return array_map(
            static function (int $index): Enumerable {
    ...
    protected static function resolve(): array
    {
        ...
        $duplicatedValues = array_filter(
            array_count_values(static::getValues()),
        ...
        $duplicatedIndices = array_filter(
            array_count_values(static::getIndices()),
            static function (int $count): bool {
                return $count > 1;
            }
        );
